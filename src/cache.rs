...
thread_local! {
  ...
  pub static SCRATCH_PATH: RefCell<PathBuf> = RefCell::new(PathBuf::with_capacity(256));
    /// Per-thread pre-allocated string buffer for string operations to avoid allocations
  pub static SCRATCH_STRING: RefCell<String> = RefCell::new(String::with_capacity(256));
