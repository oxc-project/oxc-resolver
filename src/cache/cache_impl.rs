...
use super::{
    borrowed_path::BorrowedCachedPath,
...
pub struct Cache<Fs> {
    ...
    /// Normalized cache that stores paths relative to their package roots
    normalized_paths: HashMap<(PathBuf, PathBuf), CachedPath, BuildHasherDefault<FxHasher>>,
    #[cfg(feature = "yarn_pnp")]
...
impl<Fs: FileSystem> Cache<Fs> {
    pub fn clear(&self) {
        ...
        self.normalized_paths.pin().clear();
    }
    ...
    /// Optimized cache lookup that attempts to use package-relative paths for better cache hit rate.
    /// This method tries to normalize paths relative to the nearest package.json for improved cache efficiency.
    pub(crate) fn value_normalized(&self, path: &Path) -> CachedPath {
        // First try to create the normal cached path
        let cached_path = self.value(path);

        // Try to find package root for normalization
        if let Some(package_root) = cached_path.find_package_root(&self.fs) {
            let package_root_path = package_root.to_path_buf();

            // Create relative path from package root
            if let Ok(relative_path) = path.strip_prefix(&package_root_path) {
                let normalized_key = (package_root_path, relative_path.to_path_buf());
                let normalized_cache = self.normalized_paths.pin();

                // Check if we already have this normalized entry
                if let Some(entry) = normalized_cache.get(&normalized_key) {
                    return entry.clone();
                }

                // Store the normalized entry for future lookups
                normalized_cache.insert(normalized_key, cached_path.clone());
            }
        }
    ...
    pub fn new(fs: Fs) -> Self {
        Self {
            ...
            tsconfigs: HashMap::builder()
                ...
                .resize_mode(papaya::ResizeMode::Blocking)
                .build(),
            normalized_paths: HashMap::builder()
                .hasher(BuildHasherDefault::default())
                .resize_mode(papaya::ResizeMode::Blocking)
                .build(),
            #[cfg(feature = "yarn_pnp")]
