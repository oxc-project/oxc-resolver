...
use super::{cache_impl::Cache, thread_local::SCRATCH_PATH};

#[derive(Clone)]
pub struct CachedPath(pub(crate) Arc<CachedPathImpl>);
...
pub struct CachedPathImpl {
    ...
    pub package_root: OnceLock<Option<CachedPath>>,
}
...
impl CachedPath {
    ...
    /// Find the package root for cache normalization by checking for package.json files.
    /// Returns the directory containing the nearest package.json.
    pub(crate) fn find_package_root<Fs: FileSystem>(&self, fs: &Fs) -> Option<Self> {
        self.package_root
            .get_or_init(|| {
                let mut current = Some(self);
                while let Some(cv) = current {
                    // Check if this directory contains a package.json file
                    let package_json_path = cv.path().join("package.json");
                    if fs.metadata(&package_json_path).is_ok() {
                        return Some(cv.clone());
                    }
                    current = cv.parent.as_ref();
                }
                None
            })
            .clone()
...
impl CachedPath {
    pub fn meta<Fs: FileSystem>(&self, fs: &Fs) -> Option<FileMetadata> {
